...
";
        ...
        public class Clay
        {
            ...
            public ISet<Point> Coords { get; }
        }

        private static ISet<Point> GetClayCoordinates(string input)
        {
            ...
            {
                var matchesV = Regex.Matches(line, @"x=(\d+), y=(\d+)\.\.(\d+)");
                var matchesH = Regex.Matches(line, @"y=(\d+), x=(\d+)\.\.(\d+)");

                if (matchesV.Count == 1)
                {
                    var x = int.Parse(matchesV[0].Groups[1].Value);
                    var y1 = int.Parse(matchesV[0].Groups[2].Value);
                    var y2 = int.Parse(matchesV[0].Groups[3].Value);

                    for (int i = y1; i <= y2; i++) coords.Add(new Point(x, i));
                }
                else if (matchesH.Count == 1)
                {
                    var y = int.Parse(matchesH[0].Groups[1].Value);
                    var x1 = int.Parse(matchesH[0].Groups[2].Value);
                    var x2 = int.Parse(matchesH[0].Groups[3].Value);

                    for (int i = x1; i <= x2; i++) coords.Add(new Point(i, y));
                }
                else
                {
        ...
        private void OutputGrid(Clay clay, ISet<Point> watered, ISet<Point> falling)
        {
            output.WriteLine("");

            var minx = MinOfMany(Spring.X, clay.Coords.Union(watered).Union(falling).Min(p => p.X));
            var maxx = MaxOfMany(Spring.X, clay.Coords.Union(watered).Union(falling).Max(p => p.X));
            var miny = MinOfMany(Spring.Y, clay.Coords.Union(watered).Union(falling).Min(p => p.Y));
            var maxy = MaxOfMany(Spring.Y, clay.Coords.Union(watered).Union(falling).Max(p => p.Y));
